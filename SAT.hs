...
import qualified Data.Sequence   as Q (drop, dropWhileL, empty, filter, findIndexL, fromList, index, length, singleton, takeWhileL, viewl)
...
resolve c1 c2 l = Q.fromList . S.toList . S.union c1Set $ c2Set
  where c1Set = S.delete l    . F.foldl' (\st x -> S.insert x st) S.empty $ c1
        c2Set = S.delete (-l) . F.foldl' (\st x -> S.insert x st) S.empty $ c2
